...
# 导出的数据excel
export_excel_path = "/Users/chenzuo/Desktop/code_he.xlsx"
# 需要导入的excel
import_excel_path = "/Users/chenzuo/Desktop/My-py-code/表格.xlsx"
barcodeList = []
...
for i in range(import_rows):
    if i > 3 and i < 144:
        # 12
        # print (i)
        import_barcode = arrange_string(import_table.cell(row = i, column = 12).value)
        # 如果包含这个条码
        if import_barcode in barcodeList:
            # ddd = import_table.cell(row=i, column=6).value
            import_table.cell(row=i, column=8).value = "1"
